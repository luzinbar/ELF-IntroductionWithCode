...
{
    Elf32_Shdr *first_section = (Elf32_Shdr *)((int)(s->hdr) + (s->hdr->e_shoff)); // contains pointer to the first section
    Elf32_Shdr *strtab_sctn = &first_section[s->hdr->e_shstrndx];                  // contains the entry of the strtab section
    Elf32_Shdr *dynsym_sctn, *related_symtab;
    int shstrtab_off = (((Elf32_Shdr *)((int)(s->hdr) + (s->hdr->e_shoff)))[s->hdr->e_shstrndx]).sh_offset;
    char *shstrtab_start = (map_start + shstrtab_off);
    char *relocation_sec_name = NULL;
    Elf32_Rel *relocation_table_first = NULL;
    Elf32_Sym *symbol;
    uint32_t addr_of_sym;
    int relocation_table_entries_amount = 0, relocation_table_offset = 0;
    Elf32_Sym *dymsym_first;
    for (int i = 0; i < s->hdr->e_shnum; i++)
    {
        Elf32_Shdr *curr = &first_section[i];
        char *curr_name = shstrtab_start + first_section[i].sh_name;
        if (strcmp(curr_name, ".dynstr") == 0)
        {
            related_symtab = &first_section[i];
        }
        if (curr->sh_type == SHT_DYNSYM)
        {
            dynsym_sctn = (Elf32_Shdr *)(curr->sh_offset + map_start);

            dymsym_first = (Elf32_Sym *)(curr->sh_offset + map_start);
        }

        if (curr->sh_type == SHT_REL)
        {
            relocation_sec_name = map_start + strtab_sctn->sh_offset + curr->sh_name;
            relocation_table_first = (Elf32_Rel *)(curr->sh_offset + map_start);
            relocation_table_entries_amount = curr->sh_size / curr->sh_entsize; //get amount of entries in reloctionTable
            relocation_table_offset = curr->sh_offset;
            printf("\n---RELOCATION TABLE---");
            printf("Relocation section %s at offset %x contains %d entries:\n", relocation_sec_name, relocation_table_offset, relocation_table_entries_amount);
            printf("Offset      Info        Type        Sym.Value       Sym.Name\n");
            for (int j = 0; j < relocation_table_entries_amount; j++)
            {
                Elf32_Rel *curr_rel = &((Elf32_Rel *)relocation_table_first)[j];
                printf("%08x    %08x    ", curr_rel->r_offset, curr_rel->r_info);
                addr_of_sym = ((int)s->hdr) + dynsym_sctn->sh_offset;
                // 'ELF32_R_SYM' on the info field gives you the raw data for the symbol (which is an index of the symbol in the .dynsym table where you can find its name and value).
                int sym_idx = ELF32_R_SYM(curr_rel->r_info);
                symbol = &((Elf32_Sym *)addr_of_sym)[sym_idx];
                char *sym_name = dymsym_first[sym_idx].st_name + map_start + related_symtab->sh_offset;
                // 'ELF32_R_TYPE' macro on the info field gives you the type of this relocation entry
                int sym_type = ELF32_R_TYPE(curr_rel->r_info);
                printf("%-8d    %08x        %-8s\n", sym_type, symbol->st_value, sym_name);
            }
        }
    }
}
